...
namespace OpsWay\Composer\Magento\Command;
...
class DeployCommand extends \Composer\Command\Command
{
    protected function configure()
    {
        $this
            ...
            ->setDescription('Deploy Magento core package loaded via composer.json')
            ->setDefinition(array(
            ...
            ->setHelp(<<<EOT
This command deploys magento core files from magento-core type packages.
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $io = $this->getIo();
        if ($io->isDebug()) {
            $eventManager->listen('pre-package-deploy', function($event) use ($io) {
